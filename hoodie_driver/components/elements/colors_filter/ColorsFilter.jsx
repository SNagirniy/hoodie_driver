...
const ColorsFilter = ({color_map})=> {
  const [isColorFilterOpen, setIsColorFilterOpen]= useState(false);
    ...
    return (
        <ul className={s.list}>
            {renderedColors?.map(({title, color, id,icon})=>{ return <li onClick={handleClick} id={id} key={id} className={s.item}>{color? <ColorItem color={color}/> : <IconItem icon={icon} icon_title={title}/>}<p className={s.title}>{title}</p></li>})}
